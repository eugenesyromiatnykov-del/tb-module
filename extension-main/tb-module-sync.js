...
(() => {
  ...
  // Inject 🔁 button next to the auto-toggle. Click → small popover with
  // location selector, start/pause/stop controls, progress.
  function injectBatchButton() {
    if (document.getElementById('mi-batch-btn')) return;
    const cluster = document.querySelector('#mi-header-actions');
    if (!cluster) return;
    if (typeof window.TbBatchRunner === 'undefined') return; // batch-runner not loaded

    if (!document.getElementById('mi-batch-style')) {
      const st = document.createElement('style');
      st.id = 'mi-batch-style';
      st.textContent = `
        #mi-batch-btn {
          ...
          font-size:13px !important; cursor:pointer !important; line-height:1 !important;
          font-family: inherit !important; user-select:none !important;
          padding:0 !important;
        }
        #mi-batch-btn[data-active="true"] {
          background:#10b981 !important; border-color:rgba(255,255,255,0.4) !important;
          animation: tb-batch-pulse 1.6s ease-in-out infinite;
        }
        @keyframes tb-batch-pulse {
          0%,100% { box-shadow:0 0 0 0 rgba(16,185,129,0.6); }
          50% { box-shadow:0 0 0 6px rgba(16,185,129,0); }
        }
        #mi-batch-pop {
          position:fixed !important; z-index:1000000 !important;
          background:#fff !important; color:#0f172a !important;
          border:1px solid #e2e8f0 !important; border-radius:10px !important;
          box-shadow:0 12px 32px rgba(15,23,42,0.18) !important;
          padding:12px 14px !important; min-width:280px !important;
          font: 13px -apple-system,'Segoe UI',Roboto,sans-serif !important;
        }
        #mi-batch-pop h4 { margin:0 0 8px !important; font-size:13px !important; font-weight:700 !important; }
        #mi-batch-pop label { display:flex !important; align-items:center !important; gap:6px !important; margin:4px 0 !important; font-size:12px !important; }
        #mi-batch-pop select { width:100% !important; padding:4px 6px !important; border:1px solid #cbd5e1 !important; border-radius:5px !important; font-size:12px !important; }
        #mi-batch-pop .row { display:flex !important; gap:6px !important; margin-top:8px !important; }
        #mi-batch-pop button { padding:5px 10px !important; border-radius:6px !important; border:1px solid #cbd5e1 !important; background:#f8fafc !important; cursor:pointer !important; font-size:12px !important; }
        #mi-batch-pop button.primary { background:#10b981 !important; color:white !important; border-color:#10b981 !important; }
        #mi-batch-pop button.danger { background:#fef2f2 !important; color:#b91c1c !important; border-color:#fecaca !important; }
        #mi-batch-pop .progress { font-size:12px !important; color:#475569 !important; margin:8px 0 !important; }
        #mi-batch-pop .bar { height:6px !important; background:#e2e8f0 !important; border-radius:999px !important; overflow:hidden !important; margin-top:4px !important; }
        #mi-batch-pop .bar > div { height:100% !important; background:#10b981 !important; transition:width 0.3s !important; }
        #mi-batch-pop .failed { margin-top:8px !important; font-size:11px !important; color:#b91c1c !important; max-height:80px !important; overflow:auto !important; }
      `;
      document.head.appendChild(st);
    }
    ...
    btn.type = 'button';
    btn.id = 'mi-batch-btn';
    btn.title = 'Пакетний аналіз пацієнтів';
    btn.textContent = '🔁';
    cluster.insertAdjacentElement('afterbegin', btn);
    ...
    btn.addEventListener('click', async (e) => {
      ...
      openPop(btn.getBoundingClientRect());
    });
    ...
    function openPop(rect) {
      ...
      pop.style.right = `${Math.max(8, window.innerWidth - rect.right - 4)}px`;
      pop.addEventListener('mousedown', (e) => e.stopPropagation());
      document.body.appendChild(pop);
      renderPop();
      pollTimer = setInterval(renderPop, 1500);
    }
    function closePop() {
      if (pop) pop.remove();
      pop = null;
    ...
    async function renderPop() {
      if (!pop) return;
      const s = await window.TbBatchRunner.getState();
      btn.dataset.active = String(!!s && s.status === 'running');
      if (!s || s.status === 'idle' || s.status === 'done') {
        pop.innerHTML = `
          <h4>Пакетний аналіз</h4>
          <label>
            Локація:
            <select id="mi-batch-loc">
              <option value="bilohirska">Білогірська амбулаторія</option>
              <option value="zaluzhe">Залузька амбулаторія</option>
            </select>
          </label>
          <label>
            <input type="checkbox" id="mi-batch-unsynced" checked /> тільки нові (без diagnoses_synced_at)
          </label>
          ${s && s.status === 'done' ? `<div class="progress">Минулий запуск: ${s.cursor}/${s.queue.length}, помилок ${s.failed.length}</div>` : ''}
          <div class="row">
            <button class="primary" id="mi-batch-start">Запустити</button>
            ${s && s.status === 'done' ? `<button id="mi-batch-clear">Очистити</button>` : ''}
          </div>
          <div class="progress" style="font-size:11px;">Після кліку «Запустити» MIS поверне вас на /doctors/journal. <b>Не клікайте сторінку MIS</b>, поки виконується.</div>
        `;
        pop.querySelector('#mi-batch-start')?.addEventListener('click', async () => {
          const loc = pop.querySelector('#mi-batch-loc').value;
          const onlyUnsynced = pop.querySelector('#mi-batch-unsynced').checked;
          try {
            await window.TbBatchRunner.start({ location: loc, onlyUnsynced });
          } catch (err) {
            alert(`Помилка запуску: ${err.message}`);
          }
        });
        pop.querySelector('#mi-batch-clear')?.addEventListener('click', async () => {
          await window.TbBatchRunner.clearState();
          renderPop();
        });
      } else if (s.status === 'running' || s.status === 'paused') {
        const pct = s.queue.length ? Math.round((s.cursor / s.queue.length) * 100) : 0;
        const current = s.queue[s.cursor];
        pop.innerHTML = `
          <h4>Пакетний аналіз ${s.status === 'paused' ? '— ⏸ призупинено' : '— ▶ виконується'}</h4>
          <div class="progress">
            ${s.cursor} / ${s.queue.length} · помилок ${s.failed.length}
            <div class="bar"><div style="width:${pct}%"></div></div>
          </div>
          <div class="progress">${current ? `Поточний: ${current.surname || ''} (${current.medics_id})` : '—'}</div>
          <div class="row">
            ${s.status === 'running'
              ? `<button id="mi-batch-pause">Пауза</button>`
              : `<button class="primary" id="mi-batch-resume">Продовжити</button>`}
            <button class="danger" id="mi-batch-stop">Зупинити</button>
          </div>
          ${s.failed.length > 0 ? `
            <div class="failed"><b>Не оброблені:</b><br>
              ${s.failed.slice(-8).map((f) => `${f.surname ?? ''} (${f.medics_id}): ${f.reason}`).join('<br>')}
            </div>` : ''}
        `;
        pop.querySelector('#mi-batch-pause')?.addEventListener('click', () => window.TbBatchRunner.pause());
        pop.querySelector('#mi-batch-resume')?.addEventListener('click', () => window.TbBatchRunner.resume());
        pop.querySelector('#mi-batch-stop')?.addEventListener('click', async () => {
          if (confirm('Зупинити пакетний аналіз?')) await window.TbBatchRunner.stop();
        });
      }
    }
  }
